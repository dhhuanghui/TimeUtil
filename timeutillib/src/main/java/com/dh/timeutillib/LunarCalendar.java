...
public final class LunarCalendar {
    ...
    private static int daysInLunarMonth(int year, int month) {
        if ((LUNAR_INFO[year - Constants.MIN_YEAR] & (0x100000 >> month)) == 0)
            return 29;
        else
            return 30;
    }
    ...
    static String getLunarText(int year, int month, int day) {
        String termText = SolarTermUtil.getSolarTerm(year, month, day);
        String solar = LunarCalendar.gregorianFestival(month, day);
        if (!TextUtils.isEmpty(solar))
            return solar;
        if (!TextUtils.isEmpty(termText))
            return termText;
        int[] lunar = LunarSolarConverter.solarToLunar(year, month, day);
    ...
    static void setupLunarCalendar(Calendar calendar) {
        ...
        String solarTerm = SolarTermUtil.getSolarTerm(year, month, day);
